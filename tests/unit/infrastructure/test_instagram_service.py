...
@pytest.mark.asyncio
async def test_matching_post_is_highlighted_only_during_screenshot(
    tmp_path: Path,
) -> None:
    page = MagicMock()
    page.close = AsyncMock()
    service = PlaywrightInstagramService(page)
    result_post = MagicMock()
    result_post.get_attribute = AsyncMock(return_value="/p/DcQL8sCiN1O/")
    screenshot_path = tmp_path / "1_액상비타민.png"
    ...
    with patch(
        ...
        result = await service.search_and_find_posts(
            index=1,
            keyword=Keyword(text="액상비타민"),
            posts_to_find=[
                Post(url="https://www.instagram.com/p/DcQL8sCiN1O")
            ],
            output_dir=tmp_path,
        )
    ...
    search_page.highlight_element.assert_not_called()
